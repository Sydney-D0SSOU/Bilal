...
function PnsShowcaseTile({ tile }: { tile: PnsShowcaseTileDef }) {
  const src = pnsCaseStudyImages[tile.imageKey];
  const l = (tile.left / PNS_SHOWCASE_W) * 100;
  const t = (tile.top / PNS_SHOWCASE_H) * 100;
  const w = (tile.wrapW / PNS_SHOWCASE_W) * 100;
  const h = (tile.wrapH / PNS_SHOWCASE_H) * 100;
      ...
      <div className="mx-auto flex max-w-[1248px] flex-col gap-20 px-5 py-[clamp(4rem,12vw,9.5rem)]">
        <div className="flex flex-col gap-[clamp(3rem,10vw,7.5rem)]">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <span className="font-display text-[clamp(1.25rem,4vw,2.5rem)] font-bold leading-[1.1] tracking-[4px] text-white uppercase">
              {project.title}
            </span>
          ...
          <div className="flex flex-wrap items-end justify-between gap-10">
            <div className="flex max-w-[559px] flex-col gap-10">
              ...
              <p className="text-2xl leading-8 text-neutral-300">
                <span>Refonte du site du </span>
                <span className="text-neutral-100">portail national des services publics</span>
                <span>.</span>
              </p>
            </div>
            <Link
          ...
          <div className="absolute inset-[6%_4%_6%_4%] hidden md:block">
            <div className="absolute -bottom-[10%] left-[11%] w-[59%] max-w-[735px]">
              <div className="relative aspect-[735/455] w-full overflow-hidden rounded-2xl shadow-[0_10px_15px_-3px_rgba(0,0,0,0.15)]">
                <Image
                  src={pnsCaseStudyImages.heroDesktop}
                  alt="Portail national des services publics — aperçu desktop"
                  fill
            ...
            <div className="absolute -top-[8%] right-0 w-[31%] max-w-[388px]">
              <div className="relative aspect-[388/666] w-full overflow-hidden rounded-2xl shadow-[0_10px_15px_-3px_rgba(0,0,0,0.15)]">
                <Image
                  src={pnsCaseStudyImages.heroMobile}
                  alt="Portail national des services publics — aperçu mobile"
                  fill
                  className="object-cover"
        ...
        <ScrollReveal delay={0.06}>
          ...
          <div className="flex max-w-[402px] flex-col gap-10">
            ...
            <p className="text-2xl leading-8 text-neutral-300">{pnsDetailIntro}</p>
          </div>
        ...
        >
            ...
            <DiagonalDrift className="absolute inset-0" distance={10} duration={18}>
              <div className="flex h-full items-center justify-center p-4">
                <div className="relative aspect-[9/16] h-full max-h-[280px] w-auto overflow-hidden rounded-xl shadow-lg">
                  <Image
                    src={pnsCaseStudyImages.heroMobile}
                    alt="Aperçu interface du portail"
                    fill
                    className="object-cover object-top"
                    sizes="70vw"
                  />
          ...
          <div className="relative hidden aspect-[1248/941] w-full md:block">
            ...
            <div
              ...
              style={{
                backgroundImage: `url(${pnsCaseStudyImages.noise})`,
                backgroundSize: "1024px 1024px",
              }}
            />
            <DiagonalDrift className="absolute inset-0" distance={12} duration={22} reverse>
              {pnsShowcaseTiles.map((tile, i) => (
                <PnsShowcaseTile key={i} tile={tile} />
              ))}
            </DiagonalDrift>
          </div>
          </div>
        ...
        <ScrollReveal delay={0.12}>
          ...
          <div className="flex min-w-0 w-full max-w-[604px] flex-col gap-10">
            <div className="flex w-full max-w-[402px] flex-col gap-10">
              ...
              <ul className="max-w-[500px] list-disc space-y-1 pl-9 text-2xl leading-8 text-neutral-300 marker:text-neutral-400">
                {pnsObjectifItems.map((item) => (
                  <li key={item}>{item}</li>
                ))}
              </ul>
            </div>
            <PnsDeviceFrame
              src={pnsCaseStudyImages.objectifMobileB}
              alt="Portail — profil utilisateur"
              inset={pnsDeviceInsets.right}
              className="w-full"
            />
          </div>
          </div>
